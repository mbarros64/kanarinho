...
internal class CpfCnpjNumberValidatorTest {

    @Nested
    inner class IsValidCpfNumber {
        @Test
        fun `given empty cpf, when verified if it's valid, then return false`() {
            val cpf = ""

            val isValid = CpfCnpjNumberValidator.isValidCpfNumber(cpf)
    ...
    @Nested
    inner class IsValidCnpjNumber {
        @Test
        fun `given empty cnpj, when verified if it's valid, then return false`() {
            val cnpj = ""

            val isValid = CpfCnpjNumberValidator.isValidCnpjNumber(cnpj)
